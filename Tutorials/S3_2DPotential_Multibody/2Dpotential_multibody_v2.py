...
def coupling_coefficient_cascade(t, n_midpoints):
...
"""
def vorticity_solution_kutta(mode='circulation'):
    """
...
"""
def airfoil_performance(alpha, beta, chord, vorticity, W, plot_cp=False, print_results=True):
    ...
    if print_results:
        ...
        print(f"Effective angle-of-attack  = {np.rad2deg(alpha-beta):.1f} deg (+ve means downwards tilt)")
        print(f"Circulation                = {circulation:.2f} m^2/s")
        print(f"Lift coefficient           = {CL:.2f}")
        print(f"Lift                       = {lift:.2f} N")
        print(f"\n========== End ==========\n")

    return circulation, CL, lift, Cp


def compute_flow_around_body(type, W, beta, alpha, chord, t, print_results=True):

    if type == "single":
        coupling_coefficient(n_midpoints=xmid.shape[0])
    elif type == "cascade":
        coupling_coefficient_cascade(t, n_midpoints=xmid.shape[0])
    else:
        raise RuntimeError("Coupling Coefficient type is invalid. Please make sure it is only ~single~ or ~cascade~x.")

    right_hand_side(alpha, W)
    vorticity = vorticity_solution_kutta()
    circulation, CL, lift, Cp = airfoil_performance(alpha, beta, chord, vorticity, W, plot_cp=False, print_results=print_results)
...
beta_deg = 0.0 # tilt agnle of airfoil, +ve means LE pointing downwards (rotated anti-clockwise about TE)
beta  = np.deg2rad(beta_deg)

## =============================
#  Load and prepare aerofoil data
## =============================
...
## =============================
#  Single Aerofoil (unit-solutions) calculation
## =============================
W_unit = 1.0 # [m/s]
t = chord * 0.5 # cascade pitch

""" Unit-Solution 1: U = 1.0 """
alpha_U_unit_deg = 0.0 # [deg]
alpha_U_unit = np.deg2rad(alpha_U_unit_deg)
print("Unit-Solution 1: U = 1.0 m/s")
results_u = compute_flow_around_body(type="cascade", W=W_unit, beta=beta, alpha=alpha_U_unit, chord=chord, t=t, print_results=False)

""" Unit-Solution 1: V = 1.0 """
alpha_V_unit_deg = 90.0 # [deg]
alpha_V_unit = np.deg2rad(alpha_V_unit_deg)
print("Unit-Solution 2: V = 1.0 m/s")
results_v = compute_flow_around_body(type="cascade", W=W_unit, beta=beta, alpha=alpha_V_unit, chord=chord, t=t, print_results=False)

print("Single-Aerofoil calculation is finished.\n")
...
## =============================
#  Multiple Aerofoil calculations
## =============================
print("Starting Multiple-Aerofoil calculation.\n")

results_multi = dict()

W_multi = 50.0        # freestream absolute velocity
alpha_multi_deg = 9.0 # freestream angle-of-attack 
alpha_multi = np.deg2rad(alpha_multi_deg)
U_multi = W_multi * np.cos(alpha_multi)
V_multi = W_multi * np.sin(alpha_multi)

""" Single-Aerofoil case (for results comparison) """
alpha_deg = 90.0 # [deg]
alpha = np.deg2rad(alpha_deg)
print("Single-Aerofoil case: same parameters as cascade")
results_single = compute_flow_around_body(type="single", W=W_multi, beta=beta, alpha=alpha_multi, chord=chord, t=t, print_results=True)

""" Multi-Aerofoil case (for results comparison) """
circulation_u = results_u["circulation"] # circulation of unit-solution: U = 1.0 factored by actual U-velocity value
circulation_v = results_v["circulation"] # circulation of unit-solution: V = 1.0 factored by actual U-velocity value
results_multi["circulation"] = U_multi * circulation_u + V_multi * circulation_v

# Alpha 2 - TE angle of attack
a = circulation_u/(2.0*t)
b = circulation_v/(2.0*t)
alpha_2 = np.atan( ((1-b)/(1+b))*np.tan(alpha_multi) - (2/(1+b))*a )
results_multi["alpha_2"] = np.rad2deg(alpha_2) # [deg]

# W2
results_multi["W_2"] = U_multi / np.cos(alpha_2)

# Lift Coefficient (CL)
results_multi["Lift"] = results_multi["circulation"]*rho*W_multi
results_multi["CL"]   = results_multi["Lift"] / (0.5 * rho * W_multi**2 * chord)

# Show Results
print("Multi-Aerofoil case")
print(f"========== Results ==========\n")
print(f"Freestream velocity        = {W_multi:.0f} m/s")
print(f"Freestream angle-of-attack = {alpha_multi_deg:.1f} deg")
print(f"Blade tilt angle           = {np.rad2deg(beta):.1f} deg (+ve means downwards tilt)")
print(f"Effective angle-of-attack  = {alpha_multi_deg-beta_deg:.1f} deg (+ve means downwards tilt)")
print(f"Circulation                = {results_multi["circulation"]:.2f} m^2/s")
print(f"Lift coefficient           = {results_multi["CL"]:.2f}")
print(f"Lift                       = {results_multi["Lift"]:.2f} N")
print(f"Flow angle after airfoil   = {results_multi["alpha_2"]:.2f}")
print(f"\n========== End ==========\n")

""" Results Comparison """
print(f"CL-factor (cascade/single) = {results_multi["CL"]/results_single["CL"]:.1f}\n")
