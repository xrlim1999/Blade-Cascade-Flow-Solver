...
def compute_flow_around_body(W, beta, alpha, chord, print_results=True):
...
beta_deg = 20.0 # tilt agnle of airfoil, +ve means LE pointing downwards (rotated anti-clockwise about TE)
...
results_u = compute_flow_around_body(W=W_unit, beta=beta, alpha=alpha_U_unit, chord=chord, print_results=False)

""" Unit-Solution 1: V = 1.0 """
alpha_V_unit_deg = 90.0 # [deg]
alpha_V_unit = np.deg2rad(alpha_V_unit_deg)
print("Unit-Solution 2: V = 1.0 m/s")
results_v = compute_flow_around_body(W=W_unit, beta=beta, alpha=alpha_V_unit, chord=chord, print_results=False)
...
alpha_multi_deg = 7.0 # freestream angle-of-attack 
...
results_single = compute_flow_around_body(W=W_multi, beta=beta, alpha=alpha_multi, chord=chord, print_results=True)
