...
class AirfoilSection:
    ...
    def __init__(self, filename: str, m: int, beta_deg: float):
        ...
        geom["beta"]     = np.deg2rad(beta_rad)   # blade tilt angle [deg]
...
class FlowSection:
    ...
    def __init__(self, alpha_in: float, W: float, rho: float, dynvisc: float):
        ...
        self.flow["dynvisc"] = float(dynvisc) # dynamic viscosity          [kg/s]
...
class AirfoilSolver:
    ...
    def solve_inviscid(self, plot_cp: bool, plot_save=True):
