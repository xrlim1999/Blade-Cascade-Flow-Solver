...
def vorticity_solution_kutta(mode='gamma'):
    ...
    # freestream tangential velocity components at those panels
    U_t_u = W * (np.cos(alpha)*cosine[i_u] + np.sin(alpha)*sine[i_u])
    U_t_l = W * (np.cos(alpha)*cosine[i_l] + np.sin(alpha)*sine[i_l])

    # build Kutta row: Vt_u - Vt_l = 0  ⇒  (A[i_u,:]-A[i_l,:]) @ gamma = U_t_l - U_t_u
    row = (A[i_u, :] - A[i_l, :]).copy()
    rhs_kutta = U_t_l - U_t_u

    # --- Stabilize the row scaling (prevents huge TE values) -------------------
    s = np.linalg.norm(row, ord=np.inf)  # max-abs scaling  # <<< add
    if s > 0:
        row /= s
        rhs_kutta /= s
    # --------------------------------------------------------------------------

    # Replace LAST equation with Kutta (keeps your structure)
    A[-1, :] = row
    b[-1]    = rhs_kutta
    ...
    # diagnostic: mismatch in tangential velocities at TE
    Vt_u = U_t_u + (coup[i_u, :] @ vorticity)  # use coup (not the modified row)  # <<<
    Vt_l = U_t_l + (coup[i_l, :] @ vorticity)  # <<< 
    print(f"Kutta TE Vt mismatch = {Vt_u - Vt_l:.3e}")

    # (Optional) legacy report — now just a monitor, not the actual constraint
    if mode == 'gamma':
        kutta_res = vorticity[i_u] + vorticity[i_l]
    else:
        kutta_res = vorticity[i_u]*ds[i_u] + vorticity[i_l]*ds[i_l]

    if abs(kutta_res) < 1e-6:   # relaxed tolerance since it's not the constraint  # <<<
        print("Kutta check (legacy) - OK")
    else:
...
#  Parameters
## =============
W     = 50.0 # freestream velocity [m/s]
rho   = 1.225 # freestream air density [kg/m^3]
alpha_deg = 10.0 # freestream angle-of-attack [deg]
